 ...
 */
class WPZOOM_TwitterOAuth_TwitterOAuth
{
    /** @var string */
    ...
    /** @var WPZOOM_TwitterOAuth_Consumer */
    private $consumer;
    /** @var WPZOOM_TwitterOAuth_Token */
    private $token;
    /** @var WPZOOM_TwitterOAuth_HmacSha1 */
    ...
    public function __construct($consumerKey, $consumerSecret, $oauthToken = null, $oauthTokenSecret = null)
    {
        ...
        $this->signatureMethod = new WPZOOM_TwitterOAuth_HmacSha1();
        $this->consumer = new WPZOOM_TwitterOAuth_Consumer($consumerKey, $consumerSecret);
        if (!empty($oauthToken) && !empty($oauthTokenSecret)) {
            $this->token = new WPZOOM_TwitterOAuth_Token($oauthToken, $oauthTokenSecret);
        }
    }
    ...
    public function oauth($path, array $parameters = array())
    {
        ...
        if ($this->lastHttpCode() == 200) {
            $response = WPZOOM_TwitterOAuth_Util::parseParameters($result);
            $this->lastResponse = $response;
    ...
    private function oAuthRequest($url, $method, array $parameters)
    {
        ...
        $request = WPZOOM_TwitterOAuth_Request::fromConsumerAndToken($this->consumer, $this->token, $method, $url, $parameters);
    ...
    private function request($url, $method, $headers, $postfields)
    {
        ...
        switch ($method) {
            case 'GET':
                if (!empty($postfields)) {
                    $options[CURLOPT_URL] .= '?' . WPZOOM_TwitterOAuth_Util::buildHttpQuery($postfields);
                }
                break;
            case 'POST':
                $options[CURLOPT_POST] = true;
                $options[CURLOPT_POSTFIELDS] = WPZOOM_TwitterOAuth_Util::buildHttpQuery($postfields);
                break;
        }
